...
vfov[None] = 60.0
look_v = ti.Vector(
    [lookfrom[None][0] - lookat[None][0], lookfrom[None][1] - lookat[None][1], lookfrom[None][2] - lookat[None][2]])
...
class camera:
    ...
    def reset_view(self):
        ...
        self.lower_left_corner[None] = self.origin[None] - self.horizontal[None] / 2 - self.vertical[None] / 2 - \
                                       dist_to_focus[None] * w
    ...
    def get_ray(self, u, v):
        ...
        return ray(self.origin[None] + offset,
                   self.lower_left_corner[None] + u * self.horizontal[None] + v * self.vertical[None] - self.origin[
                       None] - offset)
...
def scatter(kind, extra, r, p, normal, ff):
    ...
    if (kind == 3):  # diffuse_light
        reached = False
    if (kind == 4):  # isotropic
        r_direction = random_in_unit_sphere()
...
class hittable_list:
    ...
    def hit(self, r, t_min, t_max):
        ...
        for i in ti.static(range(len(self.objects))):
            tmp_t, tmp_p, tmp_front_face, tmp_normal, tmp_mn, tmp_ma, tmp_me = (self.objects[i]).hit(r, t_min,
                                                                                                     closest_so_far)
            if (tmp_t > 0.0):
...
@ti.data_oriented
class volumetric:
    def __init__(self, st, h, m):
        self.st = st
        self.h = h
        self.material = m

    @ti.func
    def AABB_hit(self, r, tmin, tmax):
        for i in ti.static(range(3)):
            invD = 1.0 / r.direction()[i]
            t0 = (self.st[i] - r.origin()[i]) * invD
            t1 = ((self.st + ti.Vector([self.h, self.h, self.h]))[i] - r.origin()[i]) * invD
            if (invD < 0.0):
                t0, t1 = t1, t0
            tmin = ti.max(t0, tmin)
            tmax = ti.min(t1, tmax)
    ...
    def get_dens(self, p):
        I = sample(
            (ti.Vector([ti.abs(p[0] - self.st[0]), ti.abs(p[1] - self.st[1]), ti.abs(p[2] - self.st[2])])) / self.h)
        return dens[I]

    @ti.func
    def hit(self, r, t_min, t_max):
        root = -1.0
        front_face, normal = True, ti.Vector([0.0, 0.0, 0.0])
        tmin, tmax = self.AABB_hit(r, t_min, t_max)
        t = tmin
        step_length = (tmax - tmin) / marching_steps
        if tmin < tmax:
            for steps in ti.static(range(marching_steps + 1)):
                if (ti.log(self.get_dens(r.at(t))) > 1.0 * ti.random()):
                    root = t
                t += step_length
        return root, r.at(root), front_face, normal, self.material.kind, self.material.albedo, self.material.extra
...
def gen_objects():
    ...
    material_smoke = materials(4, ti.Vector([0.1, 0.1, 0.1]), 1.0)
    world.add(volumetric(ti.Vector([-1.0, -0.5, -3.0]), 2.0, material_smoke))
...
gui = ti.GUI("Ray Tracer for Smoke", res=(image_width, image_height))
...
while gui.running:
    ...
    #         look_v = ti.Vector([lookfrom[None][0] - lookat[None][0], lookfrom[None][1] - lookat[None][1],
    #                             lookfrom[None][2] - lookat[None][2]])
    #         dist_to_focus[None] = ti.sqrt(look_v.dot(look_v))
    #         cnt = 0
    #         radience.fill(0)
    #         cam.reset_view()
    ...
    #         look_v_t = ti.Vector([look_v[0] * ti.cos(look_alpha) - look_v[2] * ti.sin(look_alpha), look_v[1],
    #                               look_v[0] * ti.sin(look_alpha) + look_v[2] * ti.cos(look_alpha)])
    #         lookfrom[None][0], lookfrom[None][1], lookfrom[None][2] = look_v_t[0] + lookat[None][0], look_v_t[1] + \
    #                                                                   lookat[None][1], look_v_t[2] + lookat[None][2]
    #         print("Lookfrom change to ", lookfrom[None])
    #         look_v = ti.Vector([lookfrom[None][0] - lookat[None][0], lookfrom[None][1] - lookat[None][1],
    #                             lookfrom[None][2] - lookat[None][2]])
    #         dist_to_focus[None] = ti.sqrt(look_v.dot(look_v))
    ...
    #         look_v_t = ti.Vector(
    #             [look_v[0] * tmp_xz_t / tmp_xz, ti.sin(look_alpha) * tmp_xz + ti.cos(look_alpha) * look_v[1],
    #              look_v[2] * tmp_xz_t / tmp_xz])
    #         lookfrom[None][0], lookfrom[None][1], lookfrom[None][2] = look_v_t[0] + lookat[None][0], look_v_t[1] + \
    #                                                                   lookat[None][1], look_v_t[2] + lookat[None][2]
    #         print("Lookfrom change to ", lookfrom[None])
    #         look_v = ti.Vector([lookfrom[None][0] - lookat[None][0], lookfrom[None][1] - lookat[None][1],
